...
// Reports & analytics data (for small embedded charts on the IT dashboard)
...
$status_colors = ['in_stock'=>'#27AE60','deployed'=>'#3498DB','under_repair'=>'#F39C12','retired'=>'#7F8C8D','disposed'=>'#7F8C8D'];

// Inventory summary per device type (totals + per-status counts + estimated value)
$inventorySummary = $pdo->query("SELECT dt.type_name,
    COUNT(d.id) AS total,
    ...
    SUM(CASE WHEN d.status IN ('retired','disposed') THEN 1 ELSE 0 END) AS retired,
    COALESCE(SUM(d.purchase_price),0) AS est_value
...
<div class="card" style="margin-top: 20px;">
    ...
    <div class="card-body" style="display: flex; gap: 12px; flex-wrap: wrap;">
        ...
        <a href="maintenance_repairs.php?tab=repairs" class="btn btn-danger" style="flex: 1; min-width: 150px;">
...
<!-- Embedded Reports & Analytics (small summaries and charts) -->
<div class="card" style="margin-top: 20px;">
    <div class="card-header">
    ...
    <div class="card-body">
        <div class="grid-2">
            <div class="card">
                <div class="card-header"><h4>Device Status Distribution</h4></div>
                <div class="card-body"><div class="chart-container"><canvas id="it_statusChart"></canvas></div></div>
            </div>
            <div class="card">
                <div class="card-header"><h4>Monthly Device Additions</h4></div>
                <div class="card-body"><div class="chart-container"><canvas id="it_monthlyChart"></canvas></div></div>
            </div>
        </div>
        <div class="grid-2" style="margin-top:12px;">
            <div class="card">
                <div class="card-header"><h4>Devices by Type</h4></div>
                <div class="card-body"><div class="chart-container"><canvas id="it_typeChart"></canvas></div></div>
            </div>
            <div class="card">
                <div class="card-header"><h4>Devices by Department</h4></div>
                <div class="card-body"><div class="chart-container"><canvas id="it_deptChart"></canvas></div></div>
            </div>
        </div>
    </div>
</div>
...
<script>
new Chart(document.getElementById('it_statusChart'), {
    type: 'doughnut',
    data: {
...
new Chart(document.getElementById('it_typeChart'), {
    type: 'bar',
    data: {
...
new Chart(document.getElementById('it_deptChart'), {
    type: 'bar',
    data: {
...
new Chart(document.getElementById('it_monthlyChart'), {
    type: 'line',
    data: {
...
function exportInventorySummaryCSV() {
    var rows = [];
    var tableRows = document.querySelectorAll('#itInventoryTable tbody tr');
    ...
    exportToCSV('it_inventory_summary_<?php echo date('Y-m-d'); ?>.csv', ['Device Type', 'Total', 'In Stock', 'Deployed', 'Under Repair', 'Retired/Disposed', 'Est. Value (PHP)'], rows);
}
</script>
...
<div class="card" style="margin-top: 20px;">
    <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; gap: 10px;">
        <h3 style="margin: 0;">Inventory Summary by Device Type</h3>
        <button class="btn btn-primary" onclick="exportInventorySummaryCSV()" type="button">
            <i class="fas fa-file-csv"></i> Export CSV
        </button>
    </div>
    <div class="card-body">
        <div class="data-table-wrapper">
            <table class="data-table" id="itInventoryTable">
                <thead>
                    <tr>
                        <th>Device Type</th>
                        <th>Total</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td><strong><?php echo sanitize($row['type_name']); ?></strong></td>
                        <td><?php echo (int)$row['total']; ?></td>
                        <td style="color: #27AE60;"><?php echo (int)$row['in_stock']; ?></td>
                        <td style="color: #3498DB;"><?php echo (int)$row['deployed']; ?></td>
                        <td style="color: #F39C12;"><?php echo (int)$row['under_repair']; ?></td>
                        <td style="color: #7F8C8D;"><?php echo (int)$row['retired']; ?></td>
                        <td><?php echo number_format($row['est_value'], 2); ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
